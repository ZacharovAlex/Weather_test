...
import 'package:weather_simple/main_state.dart';
import 'package:weather_simple/splashScreen/splash.dart';
...
import 'errors/server_not_reachable_error.dart';
      ...
      return Column(
        ...
        children: [
          ...
          if (data != null && dataForecast != null)
            NowWeatherView(
              data: data,
              trigger: nowOrForecast,
              dataForecast: dataForecast,
              dayIndex: dayIndex,
              arrayForecastingDays: arrayOfForecastDays,
            )
          else
...
//SizedBox(height: 100,),
          ...
          if (dataForecast != null)
            ...
            Expanded(
                flex: 4,
                child: TimeScrollForecast(
                  data: arrayOfForecastDays,
                  dayToShowIndex: dayIndex,
                )),

          SizedBox(
            height: 10,
          ),
          if (dataForecast != null && data != null)
            Expanded(
                flex: 7,
                child: ForecastListRow(
                    data: dataForecast, dataNow: data, arrayForecastingDays: arrayOfForecastDays))
          else
  ...
  Widget build(BuildContext context) {
    void showDialogCity() {
      showModalBottomSheet(
        ...
        builder: (_) {
          return BlocProvider.value(value: BlocProvider.of<MainCubit>(context), child: ButtonsCity());
        },
      );
    }
...
class NowWeatherView extends StatelessWidget {
  ...
  const NowWeatherView(
      {Key? key,
      required this.data,
      required this.dataForecast,
      required this.trigger,
      required this.dayIndex,
      this.arrayForecastingDays})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    if (trigger == NowOrForecast.now) {
      ...
      tempNow =
          '${dataForecast.temperatureNight(arrayForecastingDays![dayIndex]).toString()}/${dataForecast.temperatureDay(arrayForecastingDays![dayIndex]).toString()}';
                    ...
                    child: Column(
                      ...
                      children: [
                        SizedBox(
                          ...
                          child: Column(
                            children: [
                              Text('${dateFormatterWeekday(nowOrForecastDate, trigger)}',
                                  style: Theme.of(context).textTheme.caption),
                              Text('${dateFormatterDayMonth(nowOrForecastDate)}',
                                  style: Theme.of(context).textTheme.caption),
                            ],
                          ),
                        ),
...
class TimeScrollForecast extends StatelessWidget {
  ...
  const TimeScrollForecast({
    Key? key,
    this.data,
    required this.dayToShowIndex,
  }) : super(key: key);
          ...
          return Padding(
            ...
            child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    if (index == 4)
                      Text('12.00', style: Theme.of(context).textTheme.caption)
                    else
                      Text("${dateParser(data![dayToShowIndex][index].dtTxt!).hour}.00",
                          style: Theme.of(context).textTheme.caption),
                    IconName(data![dayToShowIndex][index].weather![0].id!, 50.0),
                    Text('${(data![dayToShowIndex][index].main!.temp)!.round()}' + "\u2103",
                        style: Theme.of(context).textTheme.bodyText2),
    ...
    return ListView.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
          ...
          if (index == 0) {
            ...
            pressure = pressureMmRtutniyStolb(data!.mediumPressure(arrayForecastingDays![index]),
                20); //pressure=pressureMmRtutniyStolb(dataForecast.mediumPressure(arrayForecastingDays![dayIndex]),20);
          ...
          return GestureDetector(
            onTap: () {
              scrollController.animateTo(index * 130,
            ...
            child: Padding(
              ...
              child: Container(
                ...
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                  children: [
                    Text('${dateFormatterDayMonth(dateParser(arrayForecastingDays![index][0].dtTxt!))}',
                        style: Theme.of(context).textTheme.caption),
                    Text('${weekdayParser(dateParser(arrayForecastingDays![index][0].dtTxt!))}',
                        style: Theme.of(context).textTheme.bodyText1),
                    IconName(arrayForecastingDays![index][dayIndexTodayOrForecast].weather![0].id!, 70.0),
                    //IconName(arrayForecastingDays![index][0].weather![0].id!,100.0),
                    Container(
                      child: index != 0
                          ? Column(
                              children: [
                                Row(
                                  children: [Text('Ночь : '), Text('${temperatureNight}' + "\u2103")],
                                ),
                                Row(
                                  children: [Text('День : '), Text('${temperatureDay}' + "\u2103")],
                                ),
                              ],
                            )
                          : Text('${temperatureDay}' + "\u2103"),
